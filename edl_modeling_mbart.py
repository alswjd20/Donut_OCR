...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig):
        ...
        print("여긴 뭐야? 여기도 실행이 되는건가..? MBartForConditionalGeneration 클래스 안에도 lm_head가 있는데, 같은건가..? ")

        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)

        for name, param in self.lm_head.named_parameters():
            print(" 이건 이름이 뭐야? 같나? name : ", name)
            print("shape은 뭐야? ", param.shape)
            # print(" 이건 지금 어떻게 되어 있어? ", param.require_grad)
...
class MBartForCausalLM(MBartPreTrainedModel):
    ...
    # 4번을 위해 추가
    _keys_to_ignore_on_load_missing = ["edl_layer.weight"]
    ...
    def forward(
        ...
        True
        ```"""    
        print("------------------------- MBartForCausalLM 클래스 안의 forward 함수 실행됨 -----------------------------")
        breakpoint()
