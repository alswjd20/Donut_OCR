...
class BARTDecoder(nn.Module):
    ...
    def compute_fisher_mse(self, labels_1hot_, evi_alp_):
        ...
        # breakpoint() # labels_1hot_.shape : torch.Size([33, 57580])
        # evi_alp_.shape : torch.Size([33, 57580])

        evi_alp0_ = torch.sum(evi_alp_, dim=-1, keepdim=True) # torch.Size([33, 1])

        gamma1_alp = torch.polygamma(1, evi_alp_) # torch.Size([33, 57580])
        gamma1_alp0 = torch.polygamma(1, evi_alp0_) # torch.Size([33, 1])


        gap = labels_1hot_ - evi_alp_ / evi_alp0_ # gap.shape : torch.Size([33, 57580])
        # (evi_alp_ / evi_alp0_).shape : torch.Size([33, 57580])
        # labels_1hot_.shape : torch.Size([33, 57580])

        loss_mse_ = (gap.pow(2) * gamma1_alp).sum(-1).mean() # tensor(0.9348, device = ... ) 
        loss_var_ = (evi_alp_ * (evi_alp0_ - evi_alp_) * gamma1_alp / (evi_alp0_ * evi_alp0_ * (evi_alp0_ + 1))).sum(-1).mean() # tensor(1.4184e-13, device= ...)
        # loss_det_fisher_ = - (torch.log(gamma1_alp).sum(-1) + torch.log(1.0 - (gamma1_alp0 / gamma1_alp).sum(-1))).mean() # tensor(inf, device=...)

        # loss_mse_ : 0.9348
        # loss_var_ : 1.4184e-13
        # loss_det_fisher_ : inf
    ...
    def compute_kl_loss(self, alphas, labels, target_concentration, concentration=1.0, epsilon=1e-8):
        ...
        # breakpoint() # alphas.shape : torch.Size([33, 57580])
        ...
        target_alphas = torch.ones_like(alphas) * concentration # torch.Size([33, 57580])
        target_alphas += torch.zeros_like(alphas).scatter_(-1, labels.unsqueeze(-1), target_concentration - 1) # torch.Size([33, 57580])
        # breakpoint()

        alp0 = torch.sum(alphas, dim=-1, keepdim=True) # torch.Size([33, 1])
        target_alp0 = torch.sum(target_alphas, dim=-1, keepdim=True) # torch.Size([33, 1])
        # breakpoint()

        alp0_term = torch.lgamma(alp0 + epsilon) - torch.lgamma(target_alp0 + epsilon) # torch.Size([33, 1])
        alp0_term = torch.where(torch.isfinite(alp0_term), alp0_term, torch.zeros_like(alp0_term)) # torch.Size([33, 1])
        # breakpoint()
        ...
        # breakpoint() # alphas_term.shape : torch.Size([33, 1])
        alphas_term = torch.where(torch.isfinite(alphas_term), alphas_term, torch.zeros_like(alphas_term)) # torch.Size([33, 1])
    ...
    def forward(
        ...
        evi_alp_ = torch.exp(edl_logits[0]) + 1.0
        # alphas, evi_alp의 shape : 둘 다 torch.Size([767, 57580])
        # 만약, edl_logits에 [0]을 안붙이면 -> 둘 다 torch.Size([1, 767, 57580])


        loss_mse_ = torch.tensor(0.0)
        loss_var_ = torch.tensor(0.0)
        loss_kl_ = torch.tensor(0.0)
        # loss_fisher_ = torch.tensor(0.0)
        ...
        if labels is not None: # train할때만 실행됨 
            ...
            evi_alp_ignored = evi_alp_[labels[0] != -100] # evi_alp_ignored.shape : torch.Size([33, 57580])

            y = y.to('cuda')

            # breakpoint() # y.shape : torch.Size([33, 57580]), evi_alp_ignored.shape : torch.Size([33, 57580])
            # loss_mse_, loss_var_, loss_fisher_ = self.compute_fisher_mse(y.float(), evi_alp_ignored) # 이게 원래 코드인데, loss_fisher은 inf가 나오길래 그냥 주석처리했음
            loss_mse_, loss_var_ = self.compute_fisher_mse(y.float(), evi_alp_ignored)

            evi_alp_ignored = (evi_alp_ignored - target_con) * (1 - y.float()) + target_con # torch.Size([33, 57580])

            loss_kl_ = self.compute_kl_loss(evi_alp_ignored, labels_ignored.view(-1), target_con) # 값이 굉장히 크게 나온다.. tensor(489859.9062, device=' ... )
            # breakpoint()


            if kl_c == -1:
                regr = np.minimum(1.0, epoch / 10.)
                # edl_loss = loss_mse_ + loss_var_ + fisher_c * loss_fisher_ + regr * loss_kl_ # 이게 원래 코드인데, loss_fisher_ : 지금 inf라서 주석처리함 
                edl_loss = loss_mse_ + loss_var_ + regr * loss_kl_ # + fisher_c * loss_fisher_
            else:
                # edl_loss = loss_mse_ + loss_var_ + fisher_c * loss_fisher_ + kl_c * loss_kl_ # 이게 원래 코드인데, loss_fisher_ : 지금 inf라서 주석처리함 
                edl_loss = loss_mse_ + loss_var_ + kl_c * loss_kl_ # + fisher_c * loss_fisher_
        ...
        # logit은 logit대로 내게 하고, alpha를 추가
        return ModelOutput(
            loss=loss,
            logits=logits,
...
class DonutModel(PreTrainedModel):
    ...
    def forward(self, image_tensors: torch.Tensor, decoder_input_ids: torch.Tensor, decoder_labels: torch.Tensor):
        ...
        if self.current_img_idx == 799:
            ...
            with open('train_loss_edl_loss_6_0523_iedl.json','w') as f:
    ...
    def inference(
        ...
        """ 아래는 edl로 probability를 계산한 버전 -> 이 아래처럼 그냥 decoder_output.scores를 갖고 alpha를 계산하면 안되고,
          내가 추가한 edl_layer에서 나온 애를 갖고 alpha를 계산하고, 확률을 계산해야 함 """
        # edl은 alpha를 갖고 확률을 계산.
        edl_posterior_scores = []
        # decoder_output.edl_scores : alphas를 넘겨준 것임. alpha_tensor야. Relu 취하고 1더하는 건 할 필요 없고, 바로 probability 계산하면 돼 
        # for score_tensor in decoder_output.edl_scores:
        for score_tensor in decoder_output.edl_scores: # 이건 edl_layer에서 나온 output들(edl_scores)인거지 
