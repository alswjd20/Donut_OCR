...
class GenerationMixin:
    ...
    def generate(
        ...
        output_edl_scores: Optional[bool] = None, # 4번을 위해 추가해줌 
        ...
        **model_kwargs,
        ...
        output_edl_scores = output_edl_scores if output_edl_scores is not None else self.config.output_edl_scores # 4번을 위해 추가해줌 
        ...
        if is_greedy_gen_mode:
            ...
            return self.greedy_search(
                ...
                output_edl_scores=output_edl_scores, # 4번을 위해 추가해줌 
    ...
    def greedy_search(
        ...
        output_hidden_states: Optional[bool] = None,
        output_scores: Optional[bool] = None,
        output_edl_scores: Optional[bool] = None, # 4번을 위해 추가했음 
        return_dict_in_generate: Optional[bool] = None,
        synced_gpus: Optional[bool] = False,
        **model_kwargs,
        ...
        eos_token_id = eos_token_id if eos_token_id is not None else self.config.eos_token_id
        output_scores = output_scores if output_scores is not None else self.config.output_scores
        output_edl_scores = output_edl_scores if output_edl_scores is not None else self.config.output_edl_scores # 4번을 위해 추가 
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        # init attention / hidden states / scores tuples
        scores = () if (return_dict_in_generate and output_scores) else None
        edl_scores = () if (return_dict_in_generate and output_edl_scores) else None # 4번을 위해 추가 
        decoder_attentions = () if (return_dict_in_generate and output_attentions) else None
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            print("혹시 train 할때 여기가 실행됨? utils.py 파일 안 GenerationMix 클래스 안, sample 함수안임.") # 4번 확인을 위해... 
            breakpoint()
